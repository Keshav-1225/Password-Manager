...
export async function updateData(req,res) {
    try
    {
        const updatedData = await managerSchema.findByIdAndUpdate(req.params.id,req.body,{new:true})
        if(!updatedData)
        {
...
export async function deleteData(req,res) {
    try
    {
        const deleteData = await managerSchema.findByIdAndDelete(req.params.id)
        if(!deleteData)
        {
            res.status(404).json({message:"Data not Found"})
        }
        return res.status(200).json({message:"Data deleted successfully",deleteData})
    }catch(err)
    {
